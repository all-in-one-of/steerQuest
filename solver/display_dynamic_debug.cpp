...
void drawline(){  
    ...
    if (dynamic_done < dynamic_size){
        ...
        drawCircle(objPath[dynamic_done].x, objPath[dynamic_done].y, 5.0);
        /*glBegin(GL_POINTS);
...
void DrawGLScene()
{
    ...
    drawline();
    // Detect collision with the dynamic objects
    /*vector<Boid> boid_data = flockDisplay->boids;
    cout << boid_data[1].loc.x << boid_data[1].loc.y<<endl;
    d.detectDynamicCollision(boid_data);*/
    ...
    if(sceneDisplay) {
        ...
        if(flockDisplay) {
            ...
            d.detectDynamicCollision(boids);
